    ...
    <div>
      ...
      <Card>{[0,1,2,3,4].map(i => (
        <div key={i} className="row gap-12" style={{ padding: "16px 18px", borderBottom: "1px solid var(--border-soft)" }}>
          <Skel h={32} w={32} r={9} /><div className="grow col gap-8"><Skel h={13} w="40%" r={4} /><Skel h={12} w="75%" r={4} /></div>
        </div>
      ))}</Card>
    </div>
  );
...
function EmptyInbox() {
  return <div style={{ paddingTop: 30 }}><EmptyState icon="checkcheck" title="Você está em dia 🎉"
    sub="Nenhuma notificação pendente. Avisos sobre vídeo, YouTube, newsletter e mais aparecem aqui." /></div>;
}
