...
function DraftEditor({ post, theme, onBack, go }) {
  ...
  /* ── Cowork: directions for the idea + generate the draft from the brief ── */
  const [dirsByLang, setDirsByLang] = useState({});
  const [forceWrite, setForceWrite] = useState({});
  const [genBusy, setGenBusy] = useState(false);
  const dirs = dirsByLang[activeLang] || [];
  const DIR_POOL = [
    "Abrir pela cena mais concreta e voltar pro conceito depois",
    "Enquadrar como contra-narrativa: o que todo mundo erra",
    "Conduzir pela própria experiência (story-led), não pela teoria",
    "Estruturar como um passo a passo acionável",
    "Ancorar num número/resultado que prova o ponto",
  ];
  const genMoreDirections = () => {
    setGenBusy(true);
    setTimeout(() => {
      const have = new Set(dirs);
      const next = DIR_POOL.filter(d => !have.has(d)).slice(0, 2);
      setDirsByLang(m => ({ ...m, [activeLang]: [...dirs, ...next] }));
      setGenBusy(false);
      pushToast({ kind: "info", icon: "sparkles", title: "Cowork sugeriu direções", msg: next.length ? next.length + " novas pra escolher" : "acabaram as sugestões" });
    }, 850);
  };
  const wordCount = (body) => body.reduce((n, b) => n + (b.html ? b.html.replace(/<[^>]+>/g, " ").split(/\s+/).filter(Boolean).length : 0), 0);
  const genDraft = () => {
    setGenBusy(true);
    const sinopse = (cur.sinopse || "").trim();
    const hook = (cur.hook || "").trim();
    setTimeout(() => {
      const body = [
        { t: "p", html: hook ? "<b>" + hook + "</b>" : "Abra com a promessa que prende o leitor — uma frase forte." },
        { t: "p", html: "[ Rascunho gerado pelo Cowork a partir da sinopse. Reescreva com a sua voz. ]" },
        { t: "h2", html: "O contexto" },
        { t: "p", html: sinopse || "Situe o leitor: de onde vem o problema e por que ele importa agora." },
        { t: "h2", html: "O ponto principal" },
        { t: "p", html: "Desenvolva o argumento central — a virada que torna esse artigo diferente do óbvio." },
        { t: "quote", html: "Uma frase de efeito que resume a ideia." },
        { t: "h2", html: "Como aplicar" },
        { t: "p", html: "Traduza o insight em algo acionável pra quem está lendo." },
        { t: "h2", html: "Fechamento" },
        { t: "p", html: "Amarre no hook e deixe um próximo passo claro." },
      ];
      const words = wordCount(body);
      patch({ body, words, readTime: Math.max(1, Math.round(words / 200)) + " min" });
      setForceWrite(m => ({ ...m, [activeLang]: true }));
      setGenBusy(false);
      setActiveStage("conteudo");
      pushToast({ kind: "info", icon: "sparkles", title: "Cowork rascunhou o conteúdo", msg: "Estrutura pronta — agora é escrever com a sua voz." });
    }, 1100);
  };
  const startBlank = () => {
    setForceWrite(m => ({ ...m, [activeLang]: true }));
  ...
  const restartDraft = () => {
    patch({ body: window.JSON ? JSON.parse(JSON.stringify(DEFAULT_BODY)) : DEFAULT_BODY, words: 0, readTime: "" });
    setForceWrite(m => ({ ...m, [activeLang]: false }));
    pushToast({ kind: "info", icon: "refresh", title: "Rascunho limpo", msg: "Volte a gerar com o Cowork ou comece do zero." });
  };
  ...
  const bodyEmpty = (cur.body || []).every(b => !(b.html && b.html.trim()) && !(b.t === "img" && b.status === "done"));
  const showGenChooser = isWritingStage && bodyEmpty && !cur.published && !forceWrite[activeLang];
  const cwStage = "b_" + activeStage;
  ...
  return (
    <div className="fade-in">
      ...
      <div className="ed-bar">
        ...
        {window.CoworkButton && <window.CoworkButton stage={cwStage} subtitle="Ele ajuda na ideia, no conteúdo, em imagens, SEO e publicação — peça uma alteração ou rascunho." placeholder="Ex.: rascunhe a partir da sinopse, em tom de conversa…" />}
      ...
      <div className="ed-doc">
          {isIdeaStage ? (
            ...
            <div className="idea-canvas">
              ...
              <div className="idea-dirs">
                <div className="idea-dirs-h">
                  <span className="idd-label"><Icon name="sparkles" size={12} /> Direções do Cowork</span>
                  <button className="idd-gen" onClick={genMoreDirections} disabled={genBusy}>
                    {genBusy ? <><span className="cw-spin" /> gerando…</> : <><Icon name="sparkles" size={12} /> Gerar mais</>}
                  </button>
                </div>
                {dirs.map((d, i) => (
                  <button key={i} className="idd-item" onClick={() => pushToast({ kind: "info", icon: "refresh", title: "Direção escolhida", msg: d })}>
                    <span className="idd-n">{i + 1}</span><span className="idd-t">{d}</span><span className="idd-go"><Icon name="arrowright" size={14} /></span>
                  </button>
                ))}
                {dirs.length === 0 && !genBusy && <div className="idd-empty">Sem direções ainda — peça ao Cowork pra sugerir alguns ângulos pro tema.</div>}
              </div>

              <button className="idea-next" onClick={() => setActiveStage("conteudo")}>
                {bodyEmpty ? "Gerar o conteúdo" : "Escrever o conteúdo"} <Icon name="arrowright" size={15} />
              </button>
            </div>
          ) : isWritingStage ? (
            showGenChooser ? (
            /* ── CONTEÚDO empty → generate-from-idea chooser (not a dead placeholder) ── */
            <div className="doc-gen">
              <div className="dg-kick"><Icon name="edit" size={13} /> Conteúdo · {activeLang === "pt" ? "PT-BR" : "EN"}</div>
              <h1 className="dg-title">{cur.title || "Sem título"}</h1>
              <div className="dg-brief">
                <div className="dg-bh"><span className="dg-bico"><Icon name="zap" size={12} /></span> Hook <a className="dg-edit" onClick={() => setActiveStage("ideia")}>editar</a></div>
                <div className="dg-bt">{(cur.hook || "").trim() || "Sem hook ainda — defina na etapa Ideia pra um rascunho melhor."}</div>
                {(cur.sinopse || "").trim() && <><div className="dg-bh second"><span className="dg-bico alt"><Icon name="research" size={12} /></span> Sinopse</div><div className="dg-bt">{cur.sinopse}</div></>}
              </div>
              <div className="dg-actions">
                <button className="dg-gen" onClick={genDraft} disabled={genBusy}>
                  {genBusy ? <><span className="cw-spin" /> Gerando rascunho…</> : <><Icon name="sparkles" size={16} /> Gerar rascunho com Cowork</>}
                </button>
                <button className="dg-blank" onClick={startBlank} disabled={genBusy}><Icon name="plus" size={14} /> Começar do zero</button>
              </div>
              <div className="dg-hint">O Cowork monta a estrutura a partir do hook + sinopse — você reescreve com a sua voz.</div>
            </div>
            ) : (
            ...
            <>
              ...
              <div key={"b-" + activeLang} className="doc-prose" contentEditable suppressContentEditableWarning spellCheck={false} data-empty={(cur.body || []).every(b => !b.html && b.t !== "img")}>
                {(cur.body && cur.body.length ? cur.body : DEFAULT_BODY).map((blk, i) => {
                  ...
                  if (blk.t === "h2") return <h2 key={i} dangerouslySetInnerHTML={{ __html: blk.html || "" }} />;
