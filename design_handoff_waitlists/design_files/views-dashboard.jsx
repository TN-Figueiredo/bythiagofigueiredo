...
function DashboardView({ notifs, go, onNotifAction, state }) {
  ...
  const quick = [
    ...
    { label: "Nova Ideia", sub: "Vídeo", icon: "video", dom: "pipeline" },
  ];
  const perf = [
    ...
    <div className="fade-in">
      ...
      {[["Atrasado", A.queue.overdue, "var(--danger)", "flame"], ["Hoje", A.queue.today, "var(--c-pipeline)", "clock"]].map(([label, items, color, ic]) => (
        <div key={label} style={{ marginBottom: 14 }}>
          ...
          {items.map(q => (
            <div key={q.id} className="card card-pad un-card" style={{ borderLeft: "3px solid " + color, marginBottom: 8 }}>
      ...
      <div className="grid" style={{ gridTemplateColumns: "repeat(3,1fr)" }}>
        {A.playlists.slice(0, 6).map((p, i) => (
          <Card key={i} className="card-pad pl-card">
