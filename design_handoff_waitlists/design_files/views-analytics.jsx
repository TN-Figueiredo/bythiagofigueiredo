  ...
  return (
    <div className="fade-in">
      ...
      <div className="grid g-cols-4" style={{ marginBottom: 16 }}>
        ...
        <KpiCard label="Próxima publicação" icon="clock" val="31 mai" delta={null} />
        ...
        {cells.map((c, i) => {
          ...
          return (
            <div key={i} className={"cal-cell" + (c.dimmed ? " dim" : "") + (c.today ? " today" : "")}>
              ...
              {evs.slice(0, 2).map((e, j) => { const m = EV[e.type]; return (
                <div key={j} className={"cal-ev" + (e.scheduled ? " sched" : "")} style={{ "--ec": m.c, "--es": m.s }} title={e.title}>
      ...
      <Card className="mt-16">
        ...
        {openBacklog && <div className="card-pad col gap-8 fade-in">
          {S.backlog.map((b, i) => { const m = EV[b.type]; return (
            <div key={i} className="row gap-12 bk-row" style={{ padding: "9px 12px", border: "1px solid var(--border-soft)", borderRadius: 10, background: "var(--surface-2)" }}>
