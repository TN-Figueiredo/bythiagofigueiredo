...
**Data:** 2026-09-22 · **Versão:** v12 (rodadas 1–10 + correções da implementação/deploy real; escopo 2a)
**Estado:** desenho aprovado em conversa (3 seções); spec escrito em revisão
**Mudou do v11 para o v12:** `FalhaSite("sem_chave", caminho)` corrigido (o `tipo` é o primeiro posicional, §4.6); a afirmação de que `teste_s4.py` não roda no Mac corrigida (só `teste_fila.py` depende de `httpx`, §5 F0k); `flock` do rollback do S4 ganhou `-w 1800` (§5); os comandos que invocam o worker instalado passaram de `/opt/agente/fila_intel.py` para `/opt/agente/docs/trilha/fila_intel.py` (crontab §4 e §5, canário e execução manual do F1/F4, rollback), que é o caminho real onde o kit fica depois do `scp` do card K; o comando de instalação do F1 (§4.6) virou `unset`/`export` em linhas separadas, sem o `env -u ... -u ...` de uma linha só; e §4.7 registra que `capturar_fixture.py` (F0.5) é pré-requisito do `--canario`. **Correção depois do primeiro v12:** a instalação de uma segunda cópia em `/opt/agente/fila_intel.py` (§4.6) sai do spec — na forja essa cópia nunca existiu, e duas cópias do mesmo worker é a ambiguidade que causou o crashloop; fica só `docs/trilha/fila_intel.py`, sempre (§4, §4.6, §5 rollback do S4).
**Em produção desde 2026-09-22.** Operação (desligar, religar, trocar a chave, ler o jsonl, `URL_FILA` vermelho): **`docs/ops/forja-fila-inteligencia-runbook.md`**. Onde este spec descreve o sistema *pretendido* e a forja tem outra coisa, há uma nota `> **Deriva (22/09)**` no ponto exato — 5 delas hoje (§4.1 crontab, §4.2 `series.json`, §4.6 `carregar_sitio` + kit desatualizado, §4.7 `--sombra`/`AGENTE_SITIO`, §5 rollbacks S4 e F0).
...
## 4. Forja — `/opt/agente/docs/trilha/fila_intel.py`
...
```
O caminho é `/opt/agente/docs/trilha/fila_intel.py`, não `/opt/agente/fila_intel.py`: é onde o `scp -r` do card K deixa o kit (§5), e é o caminho vivo no crontab real da forja.

> **Deriva (22/09) — o prefixo `AGENTE_SITIO=` é obrigatório enquanto o worker instalado for anterior a `646447e`.** A linha acima é a **viva** no crontab da forja, conferida em 22/09. Ela ganhou o prefixo porque o worker instalado ainda resolve o `sitio.py` com um `dirname` só (`docs/trilha/sitio.py`, que não existe) — instalar a linha **sem** o prefixo recria o crashloop de 22/09: traceback no `.err` a cada 10 min, jsonl mudo e `URL_FILA` vermelho em 70 min por `fila-jsonl-<N>s`. O prefixo deixa de ser necessário quando o worker da forja for atualizado para uma versão ≥ `646447e` do kit (`carregar_sitio()` com o `dirname(dirname(...))`, "Deriva (22/09)" do §4.6). Prova antes de tirá-lo: `ssh forja 'grep -c "dirname(os.path.dirname" /opt/agente/docs/trilha/fila_intel.py'` → `1`. Operação: `docs/ops/forja-fila-inteligencia-runbook.md`.
...
**Vídeo oculto** (`is_hidden`): **não consta em lugar nenhum** — fica fora de `canal.videos`, de `canal.views_90d`, de `canal.ultimo_video`/`dias_sem_publicar`, de série e de coorte (e da proposta de séries do F0.5). REVERTIDO em 2026-09-22 por decisão do dono: até então o oculto entrava em Σ `recent` e em `canal.videos`, e o cabeçalho contradizia o resto do relatório (35 vídeos contra os 31 vivos que série e coorte já usavam). O oculto continua listado em `feats["videos"]`: some o fato de canal, não o registro. Views que um oculto receba por link ficam de fora de propósito — `views_90d` é "views dos vídeos que o público encontra", não "views do canal". Em 18/09 são 4 dos 35 do PT: os 3 "Main AD Diamante" e "Lolzin D5" (conferido só de leitura).
...
**Série**
> **Deriva (22/09) — PENDÊNCIA ABERTA: `/opt/agente/series.json` NÃO EXISTE.** Conferido na forja em 22/09 (`ls: cannot access '/opt/agente/series.json'`), e ele também não está no kit. O F0.5 nunca chegou a produzir o arquivo. Consequência: `ler_series()` devolve `{}` → `series: []` → **o núcleo analítico desta fase roda sobre nada**. A saída publicada fica tecnicamente correta e editorialmente vazia ("Nenhuma série se afasta da coorte do mesmo período." é verdade trivial quando não há série), com **`desfecho: ok` e o check `URL_FILA` verde** — o pulso só olha `desfecho`, `claim` e `task`, então nada alarma. Foi o dono, olhando a tela, quem detectou. O canal PT tem séries óbvias nos 35 vídeos lidos em produção: `0-10 "Como somos controlados"` (11 episódios, ago–out/2019 — exatamente o caso de uso desta fase), `Vlogzeira #1–#3`, `Tailândia`, `Toronto/chegada`; `Main AD Diamante` tem 3 vídeos, mas os três `is_hidden`, e pela regra abaixo não vira série. Diagnóstico e receita de criação (captura da fixture → agrupamento, que é decisão **editorial** do dono → `scp` → conferência por `--escolher`) em `docs/ops/forja-fila-inteligencia-runbook.md` §2.
...
- **Depois da tentativa 2.** O `summary` ainda reprovado vira **template do código**, montado só com `canal` e `series`, com todo nome de série entre “ ”: "Canal com {videos} vídeos públicos no banco e {views_90d} views nos últimos 90 dias até {data_base}. Séries com efeito: “{nome}” ({razao_coorte}, {leitura}), …." (sem série: "Nenhuma série se afasta da coorte do mesmo período."; sem `recent_window`, sai a oração das views, e o template fica "Canal com {videos} vídeos públicos no banco."). O rótulo "públicos" é obrigatório desde 2026-09-22: `videos` deixou de contar os ocultos, e "N vídeos no banco" passou a ser falso sobre o banco (custo medido: +9 caracteres, 312 unidades UTF-16 de folga nos 500 do Zod). As séries entram em ordem até caber na faixa, e o template **sempre termina em `.`**, inclusive quando a faixa corta séries.
...
- **Com `fase=2` (qualquer método) ou método não-GET:**
  ...
  - exige `chave=` explícito; sem ele, `FalhaSite("sem_chave", caminho)` antes da rede — o primeiro posicional de `FalhaSite` é `tipo`, não a rota (`sitio.py:45-48`), e o próprio `sitio.py:101` já faz assim;
  - 204 → `(None, t, False)`.
...
- **`cartao.sh`** ganha, depois da linha 22, `[ "$T" = S4 ] && { (cd docs/trilha && AGENTE_SITIO="$B/sitio.py.novo" $PY -B teste_s1.py) || falha teste_s1; }`. O `teste_s2` roda depois do `deploy.sh S4`, porque o proxy carrega o `sitio.py` do próprio diretório (`trilha/s2.py:20`). O `cartao.sh` também ganha:
  ...
  - O `teste_fila.py` carrega o worker **só** por `AGENTE_FILA` (SourceFileLoader, como `replay2.py`), nunca por caminho implícito ao lado do próprio teste; quem aponta `AGENTE_FILA` é quem chama — sempre `docs/trilha/fila_intel.py`, no S4 e depois: **não existe uma segunda cópia instalada** (abaixo).
  - O `fila_intel.py` nunca faz `import sitio`. Ele carrega o módulo por caminho, por `spec_from_file_location`, como `trilha/s2.py:19-22`. O carregamento fica numa função chamada por `main()` e pelo teste. O `teste_fila` confere que o módulo carregado é o de `AGENTE_SITIO`: `ROTAS_FASE` tem a chave 2.

    > **Deriva (22/09) — o padrão descrito aqui era `os.environ.get('AGENTE_SITIO') or os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sitio.py')`, com um `dirname` só. Esse código não existe mais no kit.** Com um `dirname` só, o padrão resolvia `docs/trilha/sitio.py` — que não existe, porque o card K leva o `sitio.py` para `docs/`, não para `docs/trilha/` —, o que obrigava **todo** invocador a passar `AGENTE_SITIO`. O commit `646447e` do kit trocou por `os.path.dirname(os.path.dirname(os.path.abspath(__file__)))`, que resolve `/opt/agente/docs/sitio.py` sozinho; `AGENTE_SITIO` continua sobrescrevendo.
    >
    > **O worker instalado na forja ainda é o anterior a esse commit.** Conferido em 22/09 por `md5sum`: a forja está em `edc8cb6` (`01eba8ca…`) e o kit já passou disso. **O número de commits não instalados é móvel — no mesmo dia foi de 2 para 4** (`646447e`, `3dde429`, `d9f1079` da chave do `capturar_fixture`, `591e7c8` do `dias_sem_publicar`), e o portão `KIT-IGUAL` do card K reprovava em 4 arquivos (`fila_intel.py`, `teste_fila.py`, `teste_calculo.py`, `capturar_fixture.py`). **Não confie numa lista escrita aqui: rode o portão do K**, que é a autoridade. Enquanto isso valer, a linha do crontab precisa do prefixo `AGENTE_SITIO=` (§4.1) e todo comando manual precisa exportá-lo. Prova a qualquer momento: `md5 ~/Workspace/forja/ferramentas/docs/trilha/fila_intel.py` contra `ssh forja 'md5sum /opt/agente/docs/trilha/fila_intel.py'`. Procedimento de atualização e provas: `docs/ops/forja-fila-inteligencia-runbook.md` §2.
...
- **`fila_intel.py`** vai por scp para `docs/trilha/` e **não tem instalação — roda direto dali**, sempre. Uma segunda cópia em `/opt/agente/fila_intel.py` foi um desenho anterior; ela nunca chegou a existir na forja (o worker vivo sempre foi `/opt/agente/docs/trilha/fila_intel.py`, como prova o crontab real, §5), e duas cópias do mesmo worker são a ambiguidade — "qual delas está rodando?" — que já custou uma rodada perdida ao dono. Fica uma cópia só.
  - Antes de habilitar ou atualizar o crontab (§5), `teste_fila.py` verde **na forja**, sobre essa mesma cópia — comandos curtos, um por linha (um `env -u ... -u ... VAR=v VAR2=v2 python -B script` de uma linha só já quebrou no terminal do dono: o `env -u PYTHONPATH -u AGENTE_BASE` virou comando próprio e o `python -B` sozinho abriu um REPL):
    ```
  ...
  - Voltar uma atualização do worker: sem segunda cópia não há `.bak` de worker para restaurar — a volta é o `git checkout` da versão anterior no kit (Mac, repo git, §5) seguido de um novo `scp` (card K) e do mesmo `teste_fila.py` acima.
...
- **`--sombra --snapshot arq`:** sem claim, sem PATCH e sem fail.
  ...
  > **Deriva (22/09) — na forja, `--sombra` **sem** `--snapshot` estoura.** O `_modo_sombra` instalado chama `abrir_snapshot(args.snapshot)` sem o `or FIXTURE` que o `_modo_canario` irmão tem: sem a flag sai `TypeError` e `desfecho: bug`. Corrigido no kit em `3dde429` (o `--sombra` passa a cair na `fixture_pt.json`, igual ao `--canario`), **mas esse commit nunca foi instalado** (§4.6, "Deriva (22/09)"). Até o worker da forja ser atualizado, **passe sempre `--snapshot docs/trilha/fixture_pt.json`**. O `_modo_escolher` continua exigindo o argumento **de propósito** — ele existe para o operador inspecionar um snapshot que ele mesmo escolhe.
- **`--escolher --snapshot arq`:** sem rede, sem llama e sem `fila_intel.env`. Imprime os vídeos fora de `series.json` (todos, quando ele não existe) e a saída de `escolher` (§4.3). Roda antes do F1, pela cópia de trabalho: `cd /opt/agente && AGENTE_SITIO=/opt/agente/sitio.py venv/bin/python -B docs/trilha/fila_intel.py --escolher --snapshot docs/trilha/fixture_pt.json`. O `AGENTE_SITIO` é obrigatório aqui: sem ele o carregamento por caminho do §4.6 procura `docs/trilha/sitio.py`, que não existe (o K leva o `sitio.py` para `docs/`, não para `docs/trilha/`). O `--escolher` só usa `_t`, que o `sitio.py` da fase 1 já tem (`sitio.py:141-142`), e nunca exige `ROTAS_FASE[2]`.
  ...
  > **Deriva (22/09) — com a fase 2 no ar, use `AGENTE_SITIO=/opt/agente/docs/sitio.py`.** O caminho acima, `/opt/agente/sitio.py`, é a cópia **da fase 1** que o proxy carrega: conferido na forja, ela tem `ROTAS_FASE` = `[1]`, sem as rotas da fase 2. A frase sobre `_t` continua verdadeira — o `--escolher` roda contra ela —, mas o comando estava escrito para a janela **anterior ao S4**, e repeti-lo hoje faz o operador carregar o módulo errado em qualquer modo que precise de `ROTAS_FASE[2]`. A cópia da fase 2 é `/opt/agente/docs/sitio.py` (`ROTAS_FASE` = `[1, 2]`), e é a que a linha viva do crontab usa (§4.1). Receitas operacionais em `docs/ops/forja-fila-inteligencia-runbook.md`.
- **`--canario`:** não clama.
  ...
  - **`--canario` roda sem `--snapshot`**, e a `ENTRADA` da sonda de schema acima vem de `args.snapshot or <BASE>/docs/trilha/fixture_pt.json` — a mesma fixture que `capturar_fixture.py` (F0.5) grava e que `--sombra` usa. **Pré-requisito:** sem `fixture_pt.json` no disco o `--canario` sai `bug`. A ordem é sempre capturar fixture (F0.5) → canário (F1) → execução manual (F4 passo 1) → crontab (F4 passo 2), a mesma do §5.
...
| **F0k** | Claude, no Mac | Antes do K: escreve em `~/Workspace/forja/ferramentas/docs/trilha/` os arquivos novos `fila_intel.py` (§4), `teste_fila.py` (§4.6), `s4.py` e `teste_s4.py` (§4.6), `capturar_fixture.py` e `sonda_f0.py` (§4.7); em `~/Workspace/forja/ferramentas/fase2/` (`mkdir -p`), `pulso_f4.py` e `teste_pulso_fila.py` (§6); e altera `trilha/nova_chave.py` (desvio `--fila`, §4.6), `trilha/cartao.sh` e `trilha/deploy.sh` (§4.6) e `ferramentas/seed_chave_forja.sh` (forma `fila <sha>`, §4.6). Nada é instalado aqui: o K leva, o S4 e o F1 instalam | Bloco **F0k — para colar** (abaixo da tabela): cada `.py` novo passa em `python3 -m py_compile` e cada `.sh` em `bash -n`. `teste_fila.py` **não** roda no Mac (o `python3` do Mac não tem `httpx`, e o worker o importa — §4.1 e §4.6); ele é portão do `cartao.sh S4` e do F1, na forja. **`teste_s4.py` roda no Mac** — a justificativa acima vale só para `teste_fila.py`: `teste_s4.py` não toca no worker, testa o `sitio.py`, e roda com `AGENTE_SITIO` setado (verificado: `S4: 0 falha(s)`, 29 asserções); entra no ciclo de TDD do Mac como os demais |
| **K** | dono, no Mac | Antes do portão pós-promoção do F0, e de novo sempre que o kit mudar, **as duas linhas, sempre**: `cd ~/Workspace/forja/ferramentas/docs && scp -r sitio.py trilha forja:/opt/agente/docs/` (leva `trilha/sonda_f0.py`, `capturar_fixture.py`, `s4.py`, `teste_s4.py`, `teste_fila.py`, `teste_calculo.py`, `nova_chave.py`, `cartao.sh`, `deploy.sh` e `fila_intel.py`, como o passo 1 do S1 da fase 1) **e** `scp ~/Workspace/forja/ferramentas/fase2/pulso_f4.py ~/Workspace/forja/ferramentas/fase2/teste_pulso_fila.py forja:/opt/agente/docs/`. Claude prepara as linhas, e o dono as roda.

> **Deriva (22/09) — o K era condicional na segunda linha ("só no K que antecede o F4") e isso travou o dono ao vivo.** No F4-5 os dois arquivos do pulso não estavam na forja, o passo parou no meio do procedimento e foi preciso um `scp` avulso (PATCH 10 do ledger). **A condicional sai: o K leva tudo, sempre.** Custo de levar cedo demais: dois arquivos inertes em `/opt/agente/docs/` que ninguém invoca. Custo de não levar: o procedimento para no passo em que o dono já mexeu no `pulso.sh`.
>
> **Inventário do que a fase 2a executa na forja, por cartão** — é o que o K tem de cobrir: **F0** `trilha/sonda_f0.py`; **F0.5** `trilha/capturar_fixture.py` e `fila_intel.py --escolher`; **S4** `sitio.py` + `trilha/{s4,teste_s1,teste_s2,teste_s4,prova_site,sitio_falso,carga}.py`, `trilha/{cartao,deploy,prova,canario,portao_midia}.sh`, `trilha/fixtures_site/`, `trilha/st/`; **F1** `trilha/nova_chave.py`, `trilha/teste_fila.py`, `fila_intel.py --canario`; **F4** `fila_intel.py` e `fase2/{pulso_f4,teste_pulso_fila}.py`. Os únicos de fora de `docs/` são os dois do F4 — daí a segunda linha. `fase2/{teste_leitor_env,teste_nova_chave_fila}.py` e `teste_seed_fila.sh` **não** vão: o cabeçalho de cada um diz "rodando NO MAC", sobre tmpdir, sem tocar na forja. `seed_chave_forja.sh` também fica no Mac (escreve no banco, não na forja).
>
> **Terceiro artefato, e este o K NÃO leva de propósito:** `fase2/series.json` vai por `scp` próprio para `/opt/agente/series.json` (F0.5), **fora de `docs/`**, porque é dado curado do dono e o `scp -r trilha` não pode sobrescrevê-lo. Ele não entra no `KIT-IGUAL`; quem o guarda é a Task K-3. Hoje ele **não existe** em lugar nenhum (§4.2, "Deriva (22/09)"). O `docs/trilha/fila_intel.py` é só a cópia de trabalho; quem instala é o §4.6. A `fixture_pt.json` e o `series.json` nunca entram no kit (`ferramentas/docs/trilha/`): assim o `scp -r trilha` do K nunca sobrescreve a fixture que o dono conferiu no F0.5 | bloco **K — portão** (abaixo da tabela): **um `KIT-IGUAL` só**, que agora cobre `sitio.py` + `trilha/` **e** os dois de `fase2/`. Verde = a forja tem tudo o que a fase 2a executa; não há mais um `PULSO-IGUAL` separado que dê para esquecer |
...
# UM portão só, SEMPRE os dois conjuntos: verde aqui = a forja tem tudo o que a fase 2a executa.
...
- **F4.**
  - Tira só a linha da fila: `crontab -l | grep -vF 'docs/trilha/fila_intel.py' | crontab -`, e confere `crontab -l | grep -c fila_intel` → 0 com `retentar`/`pulso` intactos. Restaurar o `.bak-F4` inteiro apagaria linhas acrescentadas depois do F4.
...
- **S4 (dono).**
  ...
  > **Deriva (22/09) — o `proxy.py.bak-*-S4` NÃO EXISTE na forja.** `ls /opt/agente/proxy.py.bak-*-S4` devolve `No such file or directory`: os backups que o `deploy.sh` deixou são `-T1`, `-T3`, `-T4`, `-T5`, `-T6`, `-S1`, `-S2` e `-S3`, e o único `.sitio` é `proxy.py.bak-0918-2258-S1.sitio` (19/09). O rollback abaixo **falha fechada** — `B` fica vazio, o `[ -f "$B.sitio" ]` para, nada é trocado —, mas o operador lê isso como "perdi o backup" no pior momento possível. **Não perdeu:** o `sitio.py` da fase 1 está vivo em `/opt/agente/sitio.py` (`ROTAS_FASE` = `[1]`, conferido), que é exatamente o estado ao qual este rollback quer voltar; a fase 2 mora em `/opt/agente/docs/sitio.py`. Antes de rodar o bloco, **liste o que existe de fato** (`ls -la /opt/agente/proxy.py.bak-*.sitio`) e substitua `$B.sitio` pelo arquivo escolhido, ou pule direto para as provas (`ROTAS_FASE` sem a chave 2, `teste_s1.py`, `prova_site.py agente-auto` 5/5).

  O `deploy.sh` só restaura sozinho quando a saúde falha logo depois da troca (`deploy.sh:17-23`). Depois disso, com a linha do crontab já fora (rollback do F4), em `/opt/agente`: `B=$(ls proxy.py.bak-*-S4 | head -1)` (o mais antigo pelo carimbo `%m%d-%H%M` do nome: o `cp -p` do `deploy.sh` preserva o mtime, e um S4 refeito deixa um backup mais novo já com o `sitio.py` da fase 2); `[ -f "$B.sitio" ]` ou para; `flock -w 1800 /opt/agente/fila_intel.lock sh -c "cp -p $B.sitio sitio.py.tmp && mv sitio.py.tmp sitio.py" || echo 'PARE: lock ocupado por 30 min ou restauro falhou — nada foi trocado'` (o §4.6 já exige `-w 1800` em todo `flock` de shell; com o cron vivo depois do F4, uma execução segura a trava por até 25 min, e um `flock` sem limite pendura o rollback sem dizer por quê); reinicia o `proxy-agente` sem sudo, esperando o PID mudar, com a receita de `deploy.sh:13-16`: `P=$(systemctl show -p MainPID --value proxy-agente); [ "$P" -gt 0 ] && kill "$P"; N=$P; for i in $(seq 40); do N=$(systemctl show -p MainPID --value proxy-agente); [ "$N" -gt 0 ] && [ "$N" != "$P" ] && curl -fsS -m 2 127.0.0.1:8081/v1/models >/dev/null && break; sleep 1; done; [ "$N" != "$P" ] || echo 'PID nao mudou - proxy fora, avisar'`; confere `curl -fsS 127.0.0.1:8081/v1/models`, `(cd /opt/agente/docs/trilha && AGENTE_SITIO=/opt/agente/sitio.py python3 -B teste_s1.py)` sem edição, `prova_site.py agente-auto` 5/5 e `python3 -c "import importlib.util as u;s=u.spec_from_file_location('s','/opt/agente/sitio.py');m=u.module_from_spec(s);s.loader.exec_module(m);assert 2 not in m.ROTAS_FASE"` saindo 0. O `proxy.py` do S4 é cópia idêntica e não volta. Depois disso, desfaz o que o S4 e o F1 deixaram no caminho dos cartões futuros, senão o próximo cartão que escrever `sitio.py.novo` — o S5 do §10, que mexe em `resumir_canal`, função do **`sitio.py`** — reprova num portão que nada tem a ver com ele, porque sobre um `sitio.py` de volta à fase 1 o `teste_fila` não acha `ROTAS_FASE[2]`: tira a linha do `teste_fila` do `cartao.sh` vivo (`sed -i '/teste_fila/d' /opt/agente/docs/trilha/cartao.sh` e depois `grep -c teste_fila /opt/agente/docs/trilha/cartao.sh` → 0) **e** a mesma linha da cópia do kit no Mac (`~/Workspace/forja/ferramentas/docs/trilha/cartao.sh`), senão o próximo K a repõe. Não há worker instalado para remover (não existe segunda cópia, acima): `docs/trilha/fila_intel.py` sem crontab e sem chave viva fica inerte, e o rollback do F1 já apagou o `fila_intel.env`.
- **F0.** O dono:
  ...
  > **Deriva (22/09) — "`git revert` do commit" não é executável como está escrito.** O título do §3 chama o F0 de "um commit do site", mas o plano o entregou como **uma sequência de ~40 commits em `staging`**, empurrados de uma vez (push real: `06840fe3..cc39a851`, **44 commits**, ledger de 22/09), e **intercalados com trabalho de outros terminais na mesma árvore** — mockup do Health Coach, bump do tiptap, correções de analytics. Consequências obrigatórias: (a) **`git revert 06840fe3..cc39a851` está errado** — reverteria o trabalho alheio junto; (b) o rollback é o `git revert` **da lista de SHAs do F0**, na **ordem inversa**, e essa lista é a que a Task 14 anotou no relato final (ledger `.superpowers/sdd/2026-09-19-forja-fila-inteligencia-plan/progress.md`); (c) os SHAs do F0 começam em `6be83dd3` (`feat: permissao intelligence e authenticateIntel no pipeline`) e o conjunto inclui as ondas de correção posteriores (`32e614d3`, `1c7ace9a`, `6caa8869`, `4c24165e`, `1747b7b6`, `cc39a851`, …). **Levante a lista antes de reverter** (`git log --oneline --format='%h %s' 06840fe3..cc39a851` e separe o que é do F0) em vez de confiar num intervalo. Se o dono quiser mesmo um commit só, o caminho é worktree isolado + `git merge --squash` — não é o padrão deste plano.
