...
- **`fila_intel.py`** vai por scp para `docs/trilha/` e é instalado sob o lock:
  ...
  - Só depois de `teste_fila.py` passar **na forja**, a cada instalação ou atualização — comandos curtos, um por linha (um `env -u ... -u ... VAR=v VAR2=v2 python -B script` de uma linha só já quebrou no terminal do dono: o `env -u PYTHONPATH -u AGENTE_BASE` virou comando próprio e o `python -B` sozinho abriu um REPL):
    ```
    cd /opt/agente/docs/trilha
    unset PYTHONPATH AGENTE_BASE
    export AGENTE_SITIO=/opt/agente/sitio.py
