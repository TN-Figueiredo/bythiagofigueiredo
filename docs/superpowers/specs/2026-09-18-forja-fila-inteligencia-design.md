...
**Data:** 2026-09-19 · **Versão:** v7 (rodadas 1–6; escopo 2a) — a v6 (rodadas 1–5 de revisão, 8 lentes cada; patches conferidos no código e no banco) cortada ao escopo 2a por decisão do dono em 19/09, com os patches da rodada 6 (12 lentes) aplicados
...
- **O dado recente é um total móvel de 90 dias.**
  ...
  - Na linha de 18/09: Σ `recent.views` = 29, com máximo de 6 por vídeo e mediana 0 (21 dos 35 vídeos com 0). Zero inscritos em todos os vídeos. Views na vida inteira: 6.847 (valores de `view_count` congelados na importação de 06/05; o F0 os reaviva, §3.7).
...
**Efeitos**
- Nenhuma outra rota muda. Uma chave `{read, intelligence}` recebe 403 em toda escrita REST fora da fila e FORBIDDEN em toda tool MCP de escrita.
...
- **Notificação**, só **depois** de o CAS de fechamento devolver `completed`:
  ...
  - `actionHref = '/cms/youtube/analytics?channel=' + youtube_channels.channel_id`;
...
| CAS devolveu 0 linhas | inalterado; a linha de canal `forja` deste PATCH fica gravada, sem notificação (a próxima execução `forja` a sobrescreve) | 409 `TASK_NOT_RUNNING` |
...
- 404 só quando o SELECT da task devolve 0 linhas sem `error` (task de outro site ou inexistente); com `error` → 500 `INTERNAL_ERROR`, que na forja dá `fail_perdido` e deixa a task para o watchdog; 409 `TASK_NOT_RUNNING` se não está `running` ou é de outro dono.
...
- `date` = `.gte('date', hoje−3).order('date',{ascending:false}).limit(1)`;
- `recent` = `.eq('date', date)`.
...
- Os textos dos botões não mudam nesta fase.
...
- `recent_window: null` com a linha mais recente em `Date.now() - 4*864e5`; não nulo com `Date.now() - 3*864e5`.
- Linhas de vídeo de outro canal não entram. A fixture usa `youtube_videos.youtube_video_id` diferente do `id`, e `recent` só aparece quando o filtro usa o `id` (uuid).
- Um histórico com mais de 1000 linhas ainda dá o `date` mais recente e o `recent` completo.
...
- `test/youtube/yt-health-coach.test.tsx`: `baseProps` (`:43-50`) ganha `coachingMeta: null`. O teste do botão (`:113-118`, "Solicitar Nova Analise") e a interface local `CoachingCard` (`:21-29`) não mudam, porque os textos dos botões e a união do `source` ficam como estão (§3.6).
...
**`view_count` vivo.** Em `refreshMetrics` (`lib/youtube/sync.ts:152-190`, modo `metrics`, `0 12,20 * * *` em `vercel.json:14`) sai o filtro `.gte('published_at', thirtyDaysAgo)`: o modo passa a atualizar todos os vídeos do canal, em lotes de 50. Custa 1 unidade de cota por lote, e o PT gasta 1 por execução. Teste: um vídeo publicado há 400 dias recebe `view_count` novo.
...
- **Lock.** O lock é tomado pelo **próprio Python** no início de `main()`, sob `if __name__ == '__main__'`, e nunca na importação: `_TRAVA = open(os.path.join(BASE,'fila_intel.lock'),'a')`, guardado numa global de módulo que nunca é fechada, e depois `fcntl.flock(_TRAVA.fileno(), LOCK_EX|LOCK_NB)`. Um `open()` temporário seria coletado logo depois da chamada, fecharia o descritor e soltaria a trava.
  ...
  - Todo caminho vem de `BASE = os.environ.get('AGENTE_BASE','/opt/agente')` (a convenção de `replay2.py:8`: lock, `log/`, `series.json`, `roteamento.jsonl`, `fila_intel.env`, `sombra/`) e `DEFAULT = os.environ.get('AGENTE_DEFAULT','/etc/default/proxy-agente')`, lido só dentro de `main()`. `agora_mono` e `dormir` são parâmetros de `main()`, para testar o orçamento, os 9 min e as esperas de 60 s e 20 s sem esperar de verdade.
  - O `teste_fila` roda com um tmpdir em `AGENTE_BASE`/`AGENTE_DEFAULT` e confere que nenhum arquivo em `/opt/agente/log/` e nenhum lock em `/opt/agente` mudou de mtime.
...
- **Invariante de tempo.** Do claim ao último pedido ao site passam menos de 25 min + 30 s, abaixo dos 30 min do watchdog. Assim ele nunca corta uma execução viva. A rota do PATCH passa a declarar `export const maxDuration = 60` (§3.3), igual ao timeout de leitura do cliente. Com isso, quando a releitura de 20 s roda, o servidor já terminou, e `ok_inferido`/`indeterminado` refletem o estado final. Mudar o orçamento, o `timeout` ou `STALE_THRESHOLD_MINUTES` exige refazer essa conta.
...
- `desfecho` ∈ `ocupado | chat | llama_fora | config | vazia | ok | ok_inferido | reprovada | llama | falha_site | conflito | fail_perdido | chave | indeterminado | orcamento | bug | morto`.
  - `llama` = llama fora ou `truncado|timeout|json|pensou` nas duas tentativas depois do claim (`fail` com `retry`).
...
- **Não se reenvia o PATCH.** O desfecho incerto fica para a releitura e para o watchdog.
...
- `idade_dias`.
- **Maturidade.** Vídeos com menos de 90 dias ficam fora das comparações dos padrões. Acima disso, a comparação usa `view_count` direto, que passa a ser o total da vida graças ao `refreshMetrics` do F0 (§3.7). Sem essa mudança, ele fica congelado no valor da importação ou perto dos 30 dias de idade do vídeo. A saturação medida (em 18/09: +29 em 90 dias sobre 6.847) só vale para vídeos com 647 dias ou mais (o mais novo do PT, publicado em 10/12/2024, tinha 647 em 18/09); o limiar de 90 dias é revisto quando o canal voltar a publicar.
...
- `ultimo_video` = o maior `published_at` do snapshot (dd/mm/aaaa, fuso de São Paulo), e `dias_sem_publicar` é `hoje` menos essa data, em dias (§4.2, função pura). `series[].ano` = o ano da série (§4.2, episódio mediano). `leitura` = `"acima da coorte"` se `razao` ≥ 1,5, `"abaixo da coorte"` se ≤ 0,67. É calculada pelo código, e o modelo nunca deduz a direção de `razao_coorte`.
...
- **(a) Títulos entre aspas.** Sai todo trecho entre aspas (`"…"`, `“…”`, `‘…’`, `«…»`) cuja forma `_t` normalizada tenha 8 caracteres ou mais e seja substring de um título do snapshot ou de `series[].nome`.
- **(a2) Nomes de série.** Depois da etapa (a), sai **toda ocorrência literal** de `norm(_t(series[].nome))` da `ENTRADA` enviada, entre aspas ou não e **sem mínimo de tamanho**. `-`, `–` e `—` são equivalentes na comparação. Teste de tabela: `a série 0–10 rende 0,63× da coorte`, `a série '0-10'` e o template com “0–10” passam; `os vídeos 0 a 10` reprova.
...
- **(d) Por extenso.**
  ...
  - Precedidos de `menos d[aeo]|abaixo d[aeo]|mais d[aeo]|mais que|acima d[aeo]`, casam por desigualdade com um `×` permitido ("menos da metade" ⇔ algum `×` < 0,5). `quase` não abre desigualdade: "quase a metade" é o token `×` 0,5 e casa só por (f).
...
- Os itens 3–5 rodam sobre o texto já sem os títulos retirados pela etapa (a).
- Na tabela de teste, "grade de horarios" e "a nota e o titulo" passam; "tempo de exibicao" reprova.

**6. Reprovação.**
- **Tentativa 2.** Um `user` só (ver §4.4), com `corrigir: {"summary":{"anterior":"…","motivo":"…","permitidos":["35","29","0,63×","18/09/2026"]}}`.
...
- **Pouco orçamento.** Com menos de 9 min restantes, a tentativa 2 é pulada e o `summary` vai para o template.
- **`fail` só por infraestrutura ou por guarda determinística.** Com `retry` (`llama`): llama fora ou `truncado`/`timeout`/`json`/`pensou` nas duas tentativas. Sem `retry` (`reprovada`): Zod reprovado depois da montagem ou escopo 2a violado (itens 1–2), piso de dado (§4.3) ou janela ≠ 90 (§4.1 passos 4–5). Um `summary` reprovado pelo validador nunca dá `fail`: vira template.
...
- **Falha transitória no `fila_intel.py`.** Ele decide sempre por `status` (e por `code` só para separar `PARTIAL_FAILURE`), nunca por `tipo`. Uma falha é **transitória** (`retry:true` no snapshot; releitura no PATCH, "outro 5xx") quando `tipo in {'timeout','fora','429','5xx'}` **ou** quando `status == 500` e `code != 'PARTIAL_FAILURE'` (inclusive `code=None`). Um 200 com corpo `formato`/`grande` (`sitio.py:118-124`) é `falha_site` sem `retry`. O `teste_fila` tem uma linha para cada caso, e mais: 500 `INTERNAL_ERROR` no PATCH → releitura, nenhum `fail`; 500 no snapshot → `fail {retry:true}`; 500 no claim → `falha_site` com `etapa: claim`.
...
- **O `teste_fila.py` cobre no mínimo**, com `CliFalso` e llama falso:
  ...
  - llama falso que reprova o `summary` nas duas tentativas por validador (números/proibidos) → PATCH enviado uma vez com o `summary` de template, `fallback: [summary]`, `desfecho: ok` e **nenhum** `fail`; com menos de 9 min de orçamento (relógio injetado), uma chamada ao llama só;
...
- **`--escolher --snapshot arq`:** sem rede, sem llama e sem `fila_intel.env`. Imprime os vídeos fora de `series.json` (todos, quando ele não existe) e a saída de `escolher` (§4.3). Roda antes do F1, pela cópia de trabalho: `cd /opt/agente && venv/bin/python -B docs/trilha/fila_intel.py --escolher --snapshot docs/trilha/fixture_pt.json`.
- **`--canario`:** não clama.
  ...
  - Sonda a 8080 com o `S` real (só `summary`) e a `ENTRADA` da fixture: resposta válida, `reasoning_content` vazio e tok/s ≥ 80% do medido sem schema.
...
| **F2** | dono roda, Claude lê | 3× `--sombra --snapshot docs/trilha/fixture_pt.json`, só com o dado real (se a saída de `escolher` mudar, o dono confere só a diferença) | Payload final aprovado 3/3 pelo espelho **e** pelo `PatchPayloadSchema.safeParse` real (Claude roda `npx tsx` no Mac sobre os payloads copiados), sem `video_recommendations` e com `priorities: []` · `summary` aprovado na primeira geração em ≥ 2/3, e nunca em template · saída de `escolher` igual à fixture conferida · as 3 rodadas com campos numéricos idênticos · o dono lê os 3 `summary` e aprova (sem o F3, é o único julgamento de texto antes do F4) · ≤ 10 min por rodada e toda geração < 7 min |
| **F4** | dono | (1) Uma execução **manual** com um pedido PT do botão: `cd /opt/agente && timeout -k 30s 25m venv/bin/python fila_intel.py`; (2) `crontab -l > /opt/agente/crontab.bak-F4 && (cat /opt/agente/crontab.bak-F4; echo '<linha do §4.1>') \| crontab -`, conferindo que `crontab -l \| grep -c -e retentar -e pulso` não mudou; (3) o dono cria o check `URL_FILA` no healthchecks (§6) e a regra do §6 é inserida **no `pulso.sh` vivo** (a O2 é pré-requisito) por `cd /opt/agente/docs && python3 pulso_f4.py <url do check>`. Ele valida `^https://hc-ping\.com/[0-9a-f-]{36}$` e grava `pulso.sh.tmp` com o bloco inserido antes de `[ "$ok" -eq 1 ]` (âncora com contagem 1, como `trilha/s3.py:troca`), começando por `URL_FILA="<url>"`. Depois: `bash -n pulso.sh.tmp`; `teste_pulso_fila.py` verde (extrai o bloco pela âncora e o roda sobre jsonl de fixture: arquivo ausente; mtime de 50 min; última linha `modo: cron` com task `reprovada` há 2 h, e depois uma `ok` mais nova; 24 h só com `chat` → `fila-sem-claim-24h:chat`; `ok_inferido` conta como não-`ok`; em todos os casos o `ok` do pulso fica intacto); `cp -p pulso.sh pulso.sh.bak-F4`; `chmod 755 pulso.sh.tmp && mv pulso.sh.tmp pulso.sh` (o arquivo vivo é `/opt/agente/docs/pulso.sh`, num diretório do thiago; a troca por rename funciona sem sudo seja qual for o dono do arquivo) | Depois da troca, uma execução real do pulso com o check `URL_FILA` verde no healthchecks · execução manual: `desfecho: ok`, exatamente uma `youtube.intelligence_ready` com link para a página do canal e `result_summary.claimed_by` = id da `forja (fila)` · o próximo pedido PT (task do cron de segunda, ou o botão só 24 h depois do `completed_at` da manual; antes, conferir que não há `pending`/`running` PT) fica `completed` em ≤ 40 min com o llama livre e sem chat, com `result_summary.claimed_by` = id da `forja (fila)` (SQL de leitura do dono), e aparece "por forja · dd/mm" com sessão autenticada · a linha `cowork` continua no banco · o **EN** só entra em `CANAIS_FILA` com ≥ 8 vídeos e um F2 próprio |
...
- **F4.**
  ...
  - Tira só o bloco da fila do pulso com `pulso_f4.py --remover` (a mesma âncora, contagem 1; `bash -n pulso.sh.tmp`; `chmod 755 pulso.sh.tmp && mv pulso.sh.tmp pulso.sh`). Não restaura o `.bak-F4` inteiro, pelo mesmo motivo do crontab. Confere `grep -c URL_FILA /opt/agente/docs/pulso.sh` → 0.
...
- **Vazamento da `forja (fila)`.** O alcance é fechado:
  ...
  Clama só pelo REST, com `channel_ids`; não fecha tasks clamadas por outra chave, não lê além de `{read}` (mais as próprias linhas `forja` no snapshot, §3.5), não altera linhas `cowork` e não alimenta o Cowork (§3.5). A resposta é revogar (F1) e retirar (§5).
