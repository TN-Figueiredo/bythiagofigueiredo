...
**Nav:** `cms-sections.ts:34` already points the "Pipeline" item at `/cms/video` with `minRole:'editor'` and **`icon(Kanban)`** (verified — not `Video`). **Rename the label "Pipeline" → "Vídeos"** and **keep `icon(Kanban)`** — a true 1-line label-only edit. Do **NOT** switch to `icon(Video)`: it is already owned by the separate "Videos" → `/cms/youtube/videos` item at `:54`, so reusing it would create two near-identical Video-icon entries. (The `'Vídeos'` (production) vs `'Videos'` (YouTube analytics) label proximity is intentional; if it reads as a collision, pick a video-distinct icon that is NOT `Video`.) No new nav item.
...
| 7 | `pipeline-item-detail.tsx:129` (`extractMisplacedSeo`) | **No `item`/`format` in scope** — `extractMisplacedSeo(sections, lang)` is a standalone helper (`:128`). **Thread `format` in**: `extractMisplacedSeo(sections, lang, format)`; caller `:153` passes the component's `format` prop; inside, `getSectionKey('draft', lang, format)`. |
| 8 | `pipeline-item-detail.tsx:150` (`SectionPanel`) | The `format` prop here is typed **`string`** in `SectionPanelProps` (`:116`), not `Format` — call `getSectionKey(sectionType, lang, format as Format)`. |
| 9 | `tab-container.tsx:44`, `:46`, `:329` | `TabContainerProps` has a `format: Format` prop (`:16-17`), but `:44`/`:46` live inside the **standalone `hasAnyContent(def, sections)` helper** (`:37`, no format in scope). **Thread `format` in**: `hasAnyContent(def, sections, format)` (callers `:71`/`:106`/`:280` pass `format`), then `getSectionKey(key, 'en'\|'pt', format)`. `:329` is in component scope → `getSectionKey(activeTab, isShared ? 'en' : l, format)`. Count this file as **three** distinct calls. |
| — | **scope note (all 5 sites above)** | **No `item` object exists at sites 7-9** — these are `format`-prop / standalone-helper sites. Use the in-scope **`format`** (threaded into `extractMisplacedSeo`/`hasAnyContent`, cast `as Format` at `SectionPanel`), **never `item.format`**. Sites 1-6 (`items.ts`, `mcp/prompts.ts`) do have a loaded `item` and use `item.format as Format`. |
...
(The **inline** `CASE language WHEN 'en' THEN 'en' ELSE 'pt' END` maps `'pt-br'|'both'→'pt'`, `'en'→'en'` — there is **no `primary_lang()` SQL function** and **no migration** for it; inlining keeps the hub work zero-migration.) Per-card derivations (pure helpers, tested) consume **only** the projected scalars:
...
`createAbTest` (`youtube/ab-lab/actions.ts`) additionally: looks up the `youtube_videos` row by `input.youtube_video_id` (the same internal UUID we pass) at `actions.ts:109-110`, **rejects Shorts** (`duration_seconds <= 60` → error, `:119`), **rejects missing thumbnail** (`!thumbnail_hq_url` → error, `:123`), derives `original_thumbnail_url` from `thumbnail_hq_url` (preserve/immutable, `:127-130`, `:188`), seeds the original variant `is_original: true` (`:204`), and creates the `ab_tests` row with `status:'draft'`. **The 4-variant ceiling is NOT enforced inside `createAbTest`** — challengers are added via `createTextVariant`, which enforces `count >= 4` **total** (`:1048`), requires `test.status==='draft'` (`:1038`), and rejects `test_type==='thumbnail'` (`:1039`). The `:316` `>= 3` guard belongs to a *different* function that is **not on this path** — do not cite it as the ceiling.
...
- Kicker "✨ Direção · {LANG}". Editable `title` (contentEditable, writes `title_<lang>` col + payload; **empty placeholder = the authoring prompt "Título de trabalho do vídeo…"**, per handoff `views-video.jsx:244` — NOT 'Sem título', which is only the hub-card/Roteiro blank fallback). Seed card "A direção" → editable `direction`. Cowork alternatives (`siblings[]`) with "Gerar mais" (wires to Cowork, §7; prototype's local pool is the loading-state placeholder). Metadata chips: pillar (dot), Ângulos, Framework, Duração. CTA "Destrinchar em roteiro" / "Abrir o roteiro" → switch to Roteiro tab.
...
| `/cms/pipeline/research`, `/topics/[code]` | `/cms/library/research` | library (incl. the research **backend** — see §8.4: `pipeline/research/{page,actions,decision-actions,foco-actions}` + `pipeline/topics/[code]/page.tsx` move under `library/research`, with their `revalidatePath('/cms/pipeline/research')` rewritten to `/cms/library/research`) |
...
- **Research/topics backend (moves under `library/research`, NOT under `pipeline/_components/*` — must be explicitly relinked or the §8.6 deletion-gate grep stays non-zero):** `pipeline/research/actions.ts` (**10× `revalidatePath('/cms/pipeline/research')`** at lines 66/96/132/171/188/233/265/283/331/357 → `/cms/library/research`), `pipeline/research/foco-actions.ts:31`, `pipeline/research/decision-actions.ts:32`, and `pipeline/topics/[code]/page.tsx:37` (`href={`/cms/pipeline/items/${item.id}`}` → resolve via the format-aware items handler / library home). These 13 refs are a P5 relink prerequisite, not a "dies with the directory" item.
...
- [ ] Ideia: per-language title (placeholder "Título de trabalho do vídeo…") + direction editable (write `title_<lang>` + section); "Gerar mais" appends; CTA → Roteiro.
