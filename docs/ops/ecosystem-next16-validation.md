...
| `cms-admin` | 0.1.10 (consumido) | Contrato `revalidateTag: (tag: string) => void` não aceita mais o `revalidateTag` do Next 16, cujo 2º argumento (perfil) é **obrigatório** (`TS2322` em `lib/cms/admin.ts:18`). Contornado com wrapper `(tag) => revalidateTag(tag, 'seconds')`. | Média | Mudar o contrato para `(tag: string, profile?: string \| { expire?: number }) => void` e deixar o consumidor decidir o perfil, ou documentar que o pacote invalida telas de staff e fixar `'seconds'` internamente. |
| `seo` | 0.1.0 | **Declarado e não importado.** Estava em `apps/web/package.json` sem nenhum import — **removido em `0b3983f3` (2026-09-05)**; `sitemap.ts`/`robots.ts` usam `@/lib/seo/*`. Não validável por tela neste app. Ainda puxa peer de `next` no `npm ci`. | Baixa | Remover do `package.json` do consumidor (commit separado) ou validar no repo do pacote. |
| `auth-nextjs` | 2.2.0 (consumido) | `middleware.ts` mantido (não migrado para `proxy.ts`): o pacote garante "Edge Runtime safe" em `dist/middleware/create-auth-middleware.js:16`; `proxy.ts` roda só em Node. Build e typecheck passam sob Next 16 com `middleware.ts`. | Nota | Decidir o contrato (WP-6.4): manter a garantia edge e documentar que o consumidor fica em `middleware.ts`, ou publicar variante `proxy` com as implicações declaradas. |
...
- **Leitor misto → o perfil mais curto vence.** `youtube` é lida por telas de staff (`cms/youtube/page.tsx`, `lib/youtube/queries.ts`) **e** pela home pública (`lib/home/queries.ts:282,328,393,411`). O plano não previa leitor misto; a regra registrada aqui é `'seconds'` — o lado conservador (mais fresco, mais revalidação na origem), e é o que o código faz. Apontado pelo review final.
- **CORRECAO 2026-09-05 — perfis trocados por `{ expire: 0 }`.** A conversao inicial usou
  perfis nomeados (144 `'seconds'`, 28 `'minutes'`). Medido depois: perfil nomeado serve
  conteudo VELHO ate expirar (46/50 respostas velhas numa rajada logo apos invalidar),
  enquanto `{ expire: 0 }` purga na hora, como o `revalidateTag` de 1 argumento do Next 15.
  As 172 chamadas passaram a `{ expire: 0 }`. Ver `next16-wp2-lotes/CORRECAO-perfis-vs-paridade.md`.
