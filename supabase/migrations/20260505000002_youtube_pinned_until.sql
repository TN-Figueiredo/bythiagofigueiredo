...
-- Predicate uses IS NOT NULL (not > now()) because now() isn't IMMUTABLE.
-- The pin_weekly_pick RPC clears expired pins atomically before setting new ones.
DROP INDEX IF EXISTS youtube_videos_pinned_per_channel;
CREATE UNIQUE INDEX youtube_videos_pinned_per_channel
  ON youtube_videos(channel_id) WHERE pinned_until IS NOT NULL;
