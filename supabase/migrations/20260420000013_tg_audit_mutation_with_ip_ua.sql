-- Sprint 4.75: replace tg_audit_mutation to capture IP/UA via GUC.
-- Reads current_setting('app.client_ip') + current_setting('app.user_agent')
-- set by `public.set_audit_context(ip, ua)` (previous migration). Falls back
-- to inet_client_addr() if GUC unset. Tolerates malformed IP strings.
