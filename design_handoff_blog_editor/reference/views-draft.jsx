...
function DraftEditor({ post, theme, onBack, go }) {
  ...
  const [activeStage, setActiveStage] = useState(window.STAGE_MAP[post.stage] || "rascunho");
  ...
  const switchLang = (l) => {
    ...
    setContent(c => ({ ...c, [l]: { lang: l, title: "", slug: "", slugTouched: false, excerpt: "", body: window.JSON ? JSON.parse(JSON.stringify(DEFAULT_BODY)) : DEFAULT_BODY, published: false, coverReady: false, words: 0, readTime: "", publishedAt: null, updatedAt: null, dirty: false, fresh: true } }));
    setActiveLang(l);
  ...
  const setImgStatus = (id, status) => {
    if (id === "cover") { patch({ coverReady: status === "done" }); return; }
    patch({ body: (cur.body || []).map(b => (b.t === "img" && b.id === id) ? { ...b, status } : b) });
  };
  const setImgAlt = (id, alt) => patch({ body: (cur.body || []).map(b => (b.t === "img" && b.id === id) ? { ...b, alt } : b) });
  const genAll = () => patch({ coverReady: true, body: (cur.body || []).map(b => b.t === "img" ? { ...b, status: "done" } : b) });
  ...
  const isDocStage = activeStage === "ideia" || activeStage === "rascunho";
            ...
            <>
              ...
              <div className="doc-dek-wrap">
                <div key={"d-" + activeLang} className="doc-dek" contentEditable suppressContentEditableWarning spellCheck={false}
                  data-empty={!(cur.excerpt || "").trim()} onInput={onDekInput} onBlur={onDekInput}>{cur.excerpt}</div>
              ...
              <div className="doc-meta">
                ...
                <span className="msep">·</span>
                <span>{cur.readTime ? cur.readTime + " de leitura" : "rascunho novo"}</span>
                <span className="msep">·</span>
              ...
              {activeStage === "rascunho" ? (
                    ...
                    {(cur.body && cur.body.length ? cur.body : DEFAULT_BODY).map((blk, i) => {
                      if (blk.t === "img") return (
                        <div key={i} className={"doc-img " + (blk.status === "done" ? "done" : "pending")} contentEditable={false}>
                          <div className="di-thumb">{blk.status === "done" ? <div className="di-img" /> : <Icon name="media" size={20} />}</div>
                ...
                cur.fresh ? (
                  <div className="doc-prose" style={{ paddingTop: 22 }}>
                    <p style={{ color: "var(--text-dim)" }}>Esta versão ainda não tem briefing. Defina o <b style={{ color: "var(--text)" }}>hook</b> e a <b style={{ color: "var(--text)" }}>sinopse</b> próprios — cada idioma é uma versão independente, não uma tradução.</p>
                  </div>
                ) : (
                <div className="doc-prose" style={{ paddingTop: 22 }}>
                  <p style={{ color: "var(--c-pipeline)", fontWeight: 600, fontSize: 13, letterSpacing: ".4px", textTransform: "uppercase", marginBottom: 8 }}>Hook</p>
                  <p style={{ fontSize: 19, color: "var(--text)" }}>{post.hook || post.excerpt}</p>
                  <p style={{ color: "var(--text-dim)", fontWeight: 600, fontSize: 13, letterSpacing: ".4px", textTransform: "uppercase", margin: "26px 0 8px" }}>Sinopse</p>
                  <p style={{ color: "var(--text-muted)" }}>{post.sinopse || post.excerpt}</p>
                </div>
                )
              )}
            </>
