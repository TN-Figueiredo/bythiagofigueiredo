...
export async function middleware(
  ...
  // /embed/waitlists/* is the only frameable surface (see csp.ts) — same
  // prefix the X-Frame-Options negative lookahead in next.config.ts uses.
  const frameAncestors = request.nextUrl.pathname.startsWith('/embed/waitlists/') ? '*' : "'none'"
  ...
  if (mode === 'enforced' && nonceCsp) {
    ...
    response.headers.set('content-security-policy', buildLegacyCsp(isDev, frameAncestors))
