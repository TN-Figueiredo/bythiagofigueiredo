    ...
    await adapter.send({
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
          ...
          <p style="color: #525252; font-size: 14px; margin-bottom: 16px;">&ldquo;${escapeHtml(input.title)}&rdquo;</p>
          <img src="${escapeHtml(input.imageUrl)}" alt="Story preview" style="width: 100%; max-width: 270px; border-radius: 12px; margin-bottom: 16px;" />
          <div style="background: #f5f5f5; border-radius: 8px; padding: 12px 16px; margin-bottom: 16px;">
            ...
            <p style="color: #0a0a0a; font-size: 16px; font-family: monospace; margin: 0; word-break: break-all;">${escapeHtml(input.shortUrl)}</p>
          </div>
          <a href="${escapeHtml(input.readyPageUrl)}" style="display: inline-block; background: #2563eb; color: white; text-decoration: none; padding: 10px 20px; border-radius: 8px; font-size: 14px; font-weight: 500;">Open in CMS</a>
