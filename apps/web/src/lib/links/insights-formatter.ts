export interface RawInsight {
  type: 'growth' | 'decline' | 'top_performer' | 'health_warning' | 'milestone' | 'qr_surge'
  metric: string
  value: number
  period?: string
  linkTitle?: string
  ...
  switch (raw.type) {
    case 'growth':
      return {
        tone: 'up',
        icon: 'trendingUp',
        text: `Trafego de ${metricLabel} cresceu ${fmt(raw.value)}% ${raw.period ? `nos ultimos ${raw.period}` : ''}`.trim(),
      }
    case 'decline':
      return {
        tone: 'red',
        icon: 'trendingDown',
        text: `Trafego de ${metricLabel} caiu ${fmt(raw.value)}% ${raw.period ? `nos ultimos ${raw.period}` : ''}`.trim(),
      }
    case 'top_performer':
      return {
        tone: 'accent',
    ...
    case 'health_warning':
      return {
        tone: 'amber',
        icon: 'alertTriangle',
        text: `${fmt(raw.value)} ${metricLabel} com problemas de saude — verifique destinos`,
      }
    case 'milestone':
      return {
        tone: 'up',
    ...
    case 'qr_surge':
      return {
        tone: 'accent',
        icon: 'qrCode',
        text: `Pico de ${metricLabel} QR: +${fmt(raw.value)}% ${raw.period ? `nas ultimas ${raw.period}` : ''}`.trim(),
      }
    default:
      return {
        tone: 'accent',
