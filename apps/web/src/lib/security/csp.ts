 ...
 */
function sharedDirectives(frameAncestors: string): string[] {
  return [
    ...
    "object-src 'none'",
    // Path-aware: everything gets 'none' except /embed/waitlists/* (Surface 2 —
    // designed to be iframed by arbitrary third-party sites, so it gets '*';
    // the embedder set is open by definition. Clickjacking accepted there: the
    // page is only the public signup form, no authenticated actions). The
    // matching X-Frame-Options omission lives in next.config.ts (negative
    // lookahead) — XFO has no allow-all value.
 ...
 */
export function buildLegacyCsp(isDev: boolean, frameAncestors = "'none'"): string {
  const scriptSrc = isDev
    ? "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://challenges.cloudflare.com https://static.cloudflareinsights.com"
    : "script-src 'self' 'unsafe-inline' https://challenges.cloudflare.com https://static.cloudflareinsights.com"
  return ["default-src 'self'", scriptSrc, ...sharedDirectives(frameAncestors)].join('; ')
}
 ...
 */
export function buildCsp({ nonce, isDev, frameAncestors = "'none'" }: { nonce: string; isDev: boolean; frameAncestors?: string }): string {
  const scriptSrc = isDev
