...
const fetchLinksDashboardCached = unstable_cache(
...
export default async function LinksDashboardPage({ searchParams }: Props) {
  ...
  const { tree, links, analytics, latestPost: latestContentPost, latestVideo: latestContentVideo } = await fetchLinksDashboardCached(siteId, timezone)
