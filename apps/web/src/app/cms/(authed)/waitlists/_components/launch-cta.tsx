...
export function LaunchCta({ status, pending, onLaunch }: LaunchCtaProps) {
  ...
  return (
    <div className="rounded-[var(--cms-radius)] border border-cms-border bg-cms-surface p-4">
      ...
      <p id="wl-launch-reason" className="mt-1 text-xs text-cms-text-muted">{reason}</p>
      <button
        type="button"
        disabled={!enabled}
        onClick={() => onLaunch?.()}
        aria-describedby="wl-launch-reason"
        className="mt-3 inline-flex items-center gap-2 rounded-[var(--cms-radius)] bg-cms-accent px-4 py-2 text-sm font-medium text-white hover:bg-cms-accent-hover disabled:cursor-not-allowed disabled:bg-cms-surface disabled:text-cms-text-muted"
      >
        Launch broadcast
      </button>
