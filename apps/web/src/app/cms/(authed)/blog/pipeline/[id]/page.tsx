...
/**
 * Route alias: /cms/blog/pipeline/[id]
 *
 * Renders the same pipeline detail view but keeps the URL under /cms/blog/...
 * so the CMS nav correctly highlights "Blog" instead of "Pipeline".
 */
export default async function BlogPipelineItemPage({
  ...
  if (item.blog_post_id) {
    ...
    if (postRes.data) {
      ...
      const title = translations[0]?.title ?? '(sem titulo)'
