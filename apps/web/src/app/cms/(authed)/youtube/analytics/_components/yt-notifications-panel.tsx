  ...
  return (
    <div className="max-h-96 overflow-y-auto">
      <div className="flex items-center justify-between border-b border-cms-border px-3 py-2">
        ...
        <button onClick={onMarkAllRead} disabled={pending} className={`text-[10px] text-cms-accent hover:underline ${pending ? 'opacity-50 cursor-not-allowed' : ''}`}>
      ...
      <div className="divide-y divide-cms-border">
        {notifications.slice(0, 50).map(n => (
          ...
          >
            ...
            <button
              onClick={(e) => { e.stopPropagation(); if (!pending) onDismiss(n.id) }}
              disabled={pending}
              className={`shrink-0 opacity-0 group-hover:opacity-100 focus:opacity-100 text-cms-text-muted hover:text-cms-text ${pending ? 'cursor-not-allowed' : ''}`}
