...
import { mediaLocale, type SectionStatus } from '@/lib/posts/types'
...
export function ImagesTab() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        {coverUrl ? (
          <div className="relative group rounded-lg overflow-hidden" style={{ maxHeight: 280, border: '1px solid var(--gem-border)' }}>
            <img src={coverUrl} alt="Capa do post" className="w-full object-cover" style={{ maxHeight: 280 }} />
            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
              ...
              <button type="button" onClick={handleCoverRemove} aria-label="Remover capa" className="text-xs text-white bg-white/20 hover:bg-white/30 px-3 py-1.5 rounded-md"><X size={14} /></button>
            </div>
          </div>
        ) : (
          <button
      ...
      <MediaGalleryModal {...coverGallery.galleryProps} onSelect={handleCoverSelect} locale={mediaLocale(activeLocale)} siteId={post.siteId} />
    </div>
  )
}
