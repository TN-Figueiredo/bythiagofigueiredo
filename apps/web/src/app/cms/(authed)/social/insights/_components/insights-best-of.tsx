...
export function InsightsBestOf({ topThumbnails, topTitles, topPosts, strings: t }: InsightsBestOfProps) {
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
      <Podium title={t.insights.bestOf.topThumbnails} items={topThumbnails} unit="% CTR" emptyLabel={t.insights.noData} />
      <Podium title={t.insights.bestOf.topTitles} items={topTitles} unit="% CTR" emptyLabel={t.insights.noData} />
      <Podium title={t.insights.bestOf.topPosts} items={topPosts} unit=" clicks" emptyLabel={t.insights.noData} />
    </div>
  )
}

function Podium({ title, items, unit, emptyLabel }: { title: string; items: LeaderboardItem[]; unit: string; emptyLabel: string }) {
  return (
    <div className="rounded-lg border border-cms-border bg-cms-surface p-4">
      ...
      {items.length === 0 ? (
        <p className="text-xs text-cms-text-dim py-4 text-center">{emptyLabel}</p>
      ) : (
