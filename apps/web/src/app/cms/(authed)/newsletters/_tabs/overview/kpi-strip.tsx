...
const KPI_DEFS = [
  { key: 'totalSubscribers' as const, sparkKey: 'subscribers' as const, label: 'Subscribers', format: (v: number) => v.toLocaleString(), suffix: '' },
  { key: 'editionsSent' as const, sparkKey: 'editions' as const, label: 'Editions Sent', format: (v: number) => v.toString(), suffix: '' },
  { key: 'avgOpenRate' as const, sparkKey: 'openRate' as const, label: 'Open Rate', format: (v: number) => `${v.toFixed(1)}%`, suffix: 'pp' },
  { key: 'avgClickRate' as const, sparkKey: 'clickRate' as const, label: 'Click Rate', format: (v: number) => `${v.toFixed(1)}%`, suffix: 'pp' },
  { key: 'bounceRate' as const, sparkKey: 'bounceRate' as const, label: 'Bounce Rate', format: (v: number) => `${v.toFixed(1)}%`, suffix: 'pp', inverted: true },
] as const
...
export function KpiStrip({ kpis, sparklines }: KpiStripProps) {
  ...
  return (
    <div className="mb-4 flex gap-3 overflow-x-auto" role="list" aria-label="Key performance indicators">
      {KPI_DEFS.map((def) => {
        const trend = trendKeys[def.key] ?? 0
        const isPositive = 'inverted' in def ? trend < 0 : trend > 0
        return (
          <div key={def.key} role="listitem" className="flex min-w-[160px] flex-1 flex-col rounded-[10px] border border-gray-800 bg-gray-900 px-4 py-3 transition-colors hover:border-gray-700">
