...
export function TopEditions({ editions, strings }: TopEditionsProps) {
  if (editions.length === 0) return null
  const s = strings?.overview

  return (
    <div className="rounded-[10px] border border-gray-800 bg-gray-900 p-4">
      <h3 className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-gray-500">{s?.topEditions ?? 'Top Editions (30d)'}</h3>
      <div className="space-y-1">
        {editions.map((e, i) => (
          <div key={e.id} className="flex items-center gap-3 rounded-md px-2 py-1.5 hover:bg-gray-800/50">
            ...
            <span className="text-[10px] tabular-nums text-gray-500">{e.opens} {s?.opens ?? 'opens'}</span>
            <span className="text-[10px] tabular-nums text-gray-500">{e.clicks} {s?.clicks ?? 'clicks'}</span>
          </div>
        ))}
      </div>
    </div>
