...
export function DeliverabilityPanel({ data, strings }: Props) {
  ...
  return (
    <div className="rounded-[10px] border border-gray-800 bg-gray-900 p-4">
      <h3 className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-gray-500">{s?.deliverability ?? 'Deliverability'}</h3>
      <div className="flex gap-6">
        <div className="space-y-2">
          {checks.map((c) => (
            <div key={c.label} className="flex items-center gap-2">
              ...
              <span className={`text-[9px] ${c.pass ? 'text-green-400' : 'text-red-400'}`}>{c.pass ? (s?.verified ?? 'Verified') : (s?.failed ?? 'Failed')}</span>
            </div>
          ))}
        ...
        <div className="space-y-2">
          <div>
            <span className="text-[10px] text-gray-400">{strings?.kpi.bounceRate ?? 'Bounce Rate'}</span>
            ...
            <span className="text-[9px] tabular-nums text-gray-400">{data.bounceRate.toFixed(1)}% ({s?.threshold ?? 'threshold'}: 5%)</span>
          </div>
          <div>
            <span className="text-[10px] text-gray-400">{s?.complaintRate ?? 'Complaint Rate'}</span>
            <div className="mt-1 h-2 w-32 overflow-hidden rounded-full bg-gray-800">
              <div className="h-full rounded-full bg-green-500 transition-all" style={{ width: `${Math.min(100, (data.complaintRate / 0.1) * 100)}%` }} />
            </div>
            <span className="text-[9px] tabular-nums text-gray-400">{data.complaintRate.toFixed(2)}% ({s?.threshold ?? 'threshold'}: 0.1%)</span>
          </div>
        </div>
      </div>
    </div>
