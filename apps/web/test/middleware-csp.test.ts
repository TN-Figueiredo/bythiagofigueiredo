...
describe('middleware CSP — /embed/waitlists/* frameable exception', () => {
  it('legacy mode: embed path gets frame-ancestors *, other paths keep none', async () => {
    vi.stubEnv('CSP_NONCE_ENABLED', '')
    vi.stubEnv('CSP_NONCE_REPORT_ONLY', '')
    const middleware = await loadMiddleware()
    const embedRes = await middleware(makeReq('bythiagofigueiredo.com', '/embed/waitlists/some-list'))
    expect(embedRes.headers.get('content-security-policy')).toContain('frame-ancestors *')
    const rootRes = await middleware(makeReq('bythiagofigueiredo.com', '/'))
    expect(rootRes.headers.get('content-security-policy')).toContain("frame-ancestors 'none'")
  })

  it('enforced mode: embed path keeps the nonce AND frame-ancestors *', async () => {
    vi.stubEnv('CSP_NONCE_ENABLED', 'true')
    const middleware = await loadMiddleware()
    const res = await middleware(makeReq('bythiagofigueiredo.com', '/embed/waitlists/some-list'))
    const csp = res.headers.get('content-security-policy')
